    ...
    if  algorithm == 'algorithm-ffnn':       
        if useExistingModel == 'on': # For existing model
            try:
                ...
                return jsonify({'message': 'Error: Please review your optimized model.'}), 400
        elif useExistingModel == 'off': # For new dataset or train no existing model
            try:
                ...
                return jsonify({'message': 'Error: Please enter the parameters.'}), 400
        else:
            return jsonify({'message': 'Error: Please choose an available model or enter parameters.'}), 400   
#-----------LSTM-------------------   
    elif algorithm == 'algorithm-lstm':
        if useExistingModel == 'on':
            try: 
                ...
                return jsonify({'message': 'Error: Please review your optimized model.'}), 400
        elif useExistingModel == 'off': # Train a new model or use a new dataset
            try:    
...
#-----------ARIMA-------------------
        ...
        return jsonify({'message': 'Error: Please select the column and algorithm for prediction'}), 400
